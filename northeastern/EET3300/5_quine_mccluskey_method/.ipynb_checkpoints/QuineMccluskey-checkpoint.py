...
def run(minterms, dont_cares = []):
    mt = [int(i) for i in minterms]
    dc = [int(i) for i in dont_cares]
    mt.sort()
    minterms = mt+dc
    minterms.sort()
    ...
    while True:
        ...
        print("\n\n\n\nGroup   \tMinterm \t Variable State of Minterms\n%s"%('='*50))
        for i in sorted(groups.keys()):
            print("%5d:"%i) # Prints group number
            for j in groups[i]:
    ...
    print('\n\n Minimum SOP Expression: F = '+' + '.join(''.join(i) for i in final_result))
    return ret
